...
mn_cities <- st_read(here("src","ica","04_adv_maps","data","shp_loc_pop_centers",
             "city_and_township_population_centers.shp"))
mn_water <- st_read(here("src","ica","04_adv_maps","data","shp_water_lakes_rivers","LakesAndRivers.shp"))
```
...
transform_and_plot(nc, new_crs = "EPSG:32133", title = "South Carolina Ex CRS")

transform_and_plot(nc, new_crs = "EPSG:3857", title = "Web Mercator")
transform_and_plot(nc, new_crs = "EPSG:2163", title = "US National Atlas Equal Area")
...
Those points would be misaligned or offset when overlaid with the original map. 
...
ggplot(mn_counties) +
    geom_sf(fill = "white") + 
    geom_sf(data = mn_cities, size = 0.5, color = 'blue') + 
    ggthemes::theme_map()
```
...
The gradient helps better show the magnitude, because it's intuitive to grasp for the viewer. 
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

_d() is for discrete categories, _c() maps numeric values to a smooth continuous gradient, and _b() maps numeric values to a binned gradient where each range shares a single color.
...
png("04_adv_maps/figures/tc_map_zoom.png", width = 800, height = 400)
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + 
    geom_sf(data = mn_counties, fill = NA, color = "black") + 
