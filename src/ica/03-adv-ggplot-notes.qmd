...
- They want users to be able to compare the temperatures in their city to those of 2015 in order to show global temperature change over the last decade

- Visualization details like record highs, normal range, and actual range showcase the extent and drastic nature of the temp changes. 

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?
All of the normal low/highs as well as the record lows/highs
- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?
They are positioned at the forefront and highlighted in red if they're a record high or low and they're in the background if they're normal temps. 
...
ggplot(weather, aes(x = dateInYear)) +
  ...
  labs(x = "dateInYear") + theme_minimal()
...
ggplot(weather, aes(x = dateInYear)) +
  ...
  geom_vline(xintercept = month_starts, color = "grey40", linetype = "dashed") +
  labs(x = "dateInYear") + theme_minimal() 
```
...
month_centers <-
  ...
  group_by(mon) %>%
  summarize(xmin = min(dateInYear), xmax = max(dateInYear), .groups = "drop") %>%
  mutate(center = (xmin + xmax)/2)

ggplot(weather, aes(x = dateInYear)) +
  geom_ribbon(aes(ymin = NormalLow, ymax = NormalHigh), fill = "#C8B8BA") +
  geom_ribbon(aes(ymin = Low, ymax = High), fill = "#A90248") +
  # optional: month boundary lines
  geom_vline(xintercept = month_centers$xmin, linetype = "dashed", color = "grey50") +
  scale_x_continuous(
...
monthly_totals <- weather %>%
  ...
  summarize(
    ...
    center = mean(dateInYear)   # position for the label
  )

ggplot(weather, aes(x = dateInYear, y = CumPrec)) +
  # Tan fill per month
  geom_area(fill = "#ebae2e", alpha = 0.5) +
  # Blue outline
  geom_line(color = "#32a3d8") +
  # Triangles at record precipitation days
  geom_point(
    data = weather %>% filter(Precip == RecordP),
    aes(y = CumPrec),
    shape = 25, size = 3, fill = "black"
  ) +
  geom_text(
    data = monthly_totals,
    aes(x = center, y = total, label = round(total, 2)),
    vjust = -0.5, hjust = 0.5
  ) +
