...
```{r}
#1 Subset to diamonds < $400 or > $10000
diamonds_1 <- diamonds |> 
  filter(price < 400 | price > 10000)

#2 Subset to diamonds between $500 and $600 (inclusive)
diamonds_2 <- diamonds |> 
  filter(price >= 500 & price <= 600)

#3 Count Fair, Premium, or Ideal diamonds
# Wrong way with ==
# This will give a warning: "longer object length is not a multiple of shorter object length"
diamonds_wrong <- diamonds |> 
...
count_3 <- nrow(diamonds_3)
fraction_3 <- count_3 / nrow(diamonds)

#4 Fair cut > $3000? Ideal cut > $2000?
fair_over_3000 <- diamonds |> 
  filter(cut == "Fair", price > 3000)

ideal_over_2000 <- diamonds |> 
  filter(cut == "Ideal", price > 2000)

any_fair_over_3000 <- nrow(fair_over_3000) > 0
all_ideal_over_2000 <- all(ideal_over_2000$price > 2000)

#5 Create categorized versions of price
diamonds_5 <- diamonds |> 
  mutate(
...
# Create factor with sensible order
ratings_factor <- factor(
  ratings,
